 ...
 * RLS wirkt (nur die eigene Zeile). Fehlende Zeile = Vorgabe „aus" (Opt-in): Fristen-Mails
 * gehen NUR raus, wenn der Nutzer sie ausdrücklich aktiviert hat. Sprache 'de'.
...
export async function loadReminderSettings(userId: string): Promise<ReminderSettings> {
  ...
  return { enabled: data?.enabled ?? false, locale: (data?.locale as "de" | "en") ?? "de" };
}
