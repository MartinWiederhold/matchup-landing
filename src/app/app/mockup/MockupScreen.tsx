    ...
    <div className="min-h-[100dvh] w-full bg-black text-white">
      <div className="relative mx-auto min-h-[100dvh] w-full max-w-[430px] overflow-hidden bg-black px-5 pb-32 pt-[max(20px,env(safe-area-inset-top))]">
        ...
        <div className="mt-6 rounded-[24px] bg-white/[0.045] p-5">
          <button type="button" onClick={() => openComposer(false)} className="block w-full text-left text-[17px] text-zinc-500">
            Start a post
          </button>
          <div className="mt-6 flex items-center justify-between">
            <button type="button" onClick={() => openComposer(false)} className="flex items-center gap-2 rounded-full bg-white/[0.06] py-2.5 pl-3 pr-4 text-sm font-semibold text-white">
            ...
            <div className="flex gap-2.5">
              <button type="button" onClick={() => openComposer(true)} className="flex h-11 w-11 items-center justify-center rounded-full bg-white/[0.06] text-white">
        ...
        {posts.length > 0 && (
          <div className="mt-4 space-y-4">
            {posts.map((p) => (
              <article key={p.id} className="overflow-hidden rounded-[24px] bg-white/[0.045]">
                <div className="flex items-center gap-3 px-5 pt-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-zinc-500">{p.time}</p>
                  </div>
                </div>
                {p.text && <p className="px-5 pt-3 text-[15px] leading-snug text-zinc-200">{p.text}</p>}
                {p.image && <img src={p.image} alt="" className="mt-3 max-h-[420px] w-full object-cover" />}
                <div className="flex items-center gap-5 px-5 py-3.5 text-zinc-400">
        ...
        {composerOpen && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 backdrop-blur-sm" onClick={closeComposer}>
            <div className="mx-auto w-full max-w-[430px]" onClick={(e) => e.stopPropagation()}>
              <div className="rounded-t-[28px] border-t border-white/10 bg-[#141416] p-5 pb-8">
                <div className="mb-4 flex items-center justify-between">
                  <button type="button" onClick={closeComposer} className="text-sm font-medium text-zinc-400">Abbrechen</button>
                  <span className="text-sm font-bold text-white">Neuer Post</span>
                  <button
                    type="button"
                    onClick={publish}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="min-h-[80px] flex-1 resize-none bg-transparent text-[16px] text-white outline-none placeholder:text-zinc-500"
                  />
                </div>
                {draftImg && (
                ...
                <div className="mt-4 flex items-center gap-3 border-t border-white/10 pt-4">
                  <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-2 rounded-full bg-white/[0.06] px-3.5 py-2 text-sm font-semibold text-white">
