...
export default function TabBar({
  ...
  // Bei Center-Button die Tabs links/rechts aufteilen; der Button sitzt erhöht in der Mitte.
  const mid = center ? Math.ceil(tabs.length / 2) : tabs.length;
  const leftTabs = tabs.slice(0, mid);
  const rightTabs = center ? tabs.slice(mid) : [];

  // Jeder Tab sitzt in einem FIXEN 56px-Slot. Nur das innere Pill waechst/schrumpft
  // beim Aktivieren — die Slot-Breite bleibt gleich, darum verrutscht nichts mehr.
  // (Vorher aenderte der aktive Button selbst seine Breite von 48 auf 56px, was in
  //  der justify-between-Reihe alle anderen Icons verschob.)
      ...
      <div key={tab.key} className="flex h-14 w-14 shrink-0 items-center justify-center">
        <button
          ...
          className={`relative flex items-center justify-center rounded-full transition-all duration-300 ${
            isActive ? "h-14 w-14 bg-gradient-to-br from-matchup to-indigo-500 text-white shadow-lg" : "h-12 w-12 text-neutral-400"
          }`}
        >
          <svg width={isActive ? 22 : 23} height={isActive ? 22 : 23} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d={tab.icon} />
          </svg>
          {badge > 0 && (
  ...
  if (center) {
    return (
      <div className={`pointer-events-none fixed inset-x-0 bottom-0 z-30 flex justify-center px-4 pb-[max(14px,env(safe-area-inset-bottom))] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${hidden ? "translate-y-[160%] opacity-0" : "translate-y-0 opacity-100"}`}>
