...
const SERIES_STYLE: Record<string, { bar: string; text: string; dot: string }> = {
  itf_wtt: { bar: "bg-[var(--t2-accent)]/[0.12] ring-matchup/30", text: "text-[var(--t2-accent)]", dot: "bg-[var(--t2-accent)]" },
  itf_juniors: { bar: "bg-[var(--t2-success)]/[0.12] ring-[var(--t2-success)]/30", text: "text-[var(--t2-success)]", dot: "bg-[var(--t2-success)]" },
  challenger: { bar: "bg-[var(--t2-warn-surface)] ring-[var(--t2-warn)]", text: "text-[var(--t2-warn)]", dot: "bg-[var(--t2-warn)]" },
  wta: { bar: "bg-[var(--t2-danger)]/[0.12] ring-[var(--t2-danger)]/30", text: "text-[var(--t2-danger)]", dot: "bg-[var(--t2-danger)]" },
};
...
const SERIES_BADGE: Record<string, string> = {
  itf_wtt: "text-[var(--t2-accent)] bg-[var(--t2-accent)]/10",
  itf_juniors: "text-[var(--t2-success)] bg-[var(--t2-success-surface)]",
  challenger: "text-[var(--t2-warn)] bg-[var(--t2-warn-surface)]",
  wta: "text-[var(--t2-danger)] bg-[var(--t2-danger)]/10",
};
...
function Face({ src, name }: { src: string | null; name: string | null }) {
  ...
  return <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--t2-accent)]/15 t2-fs-meta font-bold text-[var(--t2-accent)] ring-2 ring-white">{(name?.[0] ?? "·").toUpperCase()}</span>;
}
...
export default function TimelineView() {
    ...
    <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
      {!isMobile && (
        <div className="flex items-center gap-0.5 rounded-full bg-[var(--t2-surface)] p-0.5">
          {(["season", "month", "week"] as Zoom[]).map((z) => (
            <button key={z} type="button" onClick={() => { setZoom(z); didInitialScroll.current = false; }} className={`rounded-full px-3 py-1 t2-fs-micro font-semibold transition-colors ${zoom === z ? "bg-[var(--t2-paper)] text-[var(--t2-ink)] shadow-sm" : "text-[var(--t2-muted)] hover:text-[var(--t2-ink)]"}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        {tMondays.length > 0 && !isMobile && (
          <button type="button" onClick={scrollToFocus} className="rounded-full bg-[var(--t2-accent)]/[0.08] px-3.5 py-1.5 t2-fs-micro font-bold text-[var(--t2-accent)] ring-1 ring-matchup/20 hover:bg-[var(--t2-accent)]/[0.14]">{t("tour.tlJumpNext")} →</button>
        )}
      </div>
    </div>
  );
  ...
  if (isMobile) {
    if (activeSeason.length === 0 && events.length === 0) return <div>{controls}{eventForm}<p className="mt-6 rounded-xl bg-[var(--t2-surface)] px-5 py-8 text-center t2-fs-body text-[var(--t2-muted)]">{t("tour.wsSeasonEmpty")}</p></div>;
    ...
    return (
      <div className="w-full">{controls}{eventForm}
        <ol className="mt-5 space-y-2 border-l-2 border-[var(--t2-text)]/[0.08] pl-4">
          {weeks.map((wk) => {
              ...
              <li key={wk} className="relative">
                ...
                {tt && (
                  <button type="button" onClick={() => setSelected(tt.id)} className="mt-1 block w-full rounded-xl bg-[var(--t2-paper)] px-3 py-2 text-left shadow-sm ring-1 ring-[var(--t2-line)]">
                    ...
                    <span className="mt-1 flex flex-wrap items-center gap-2"><DeadlineChip kind={dl?.kind ?? "unknown"} entryMs={dl?.entryMs ?? null} nowMs={nowMs} t={t} />{tight.has(tt.id) && <span className="t2-fs-meta font-semibold text-[var(--t2-warn)]">⚠ {t("tour.calTightArrival", { n: tight.get(tt.id)! })}</span>}</span>
                  </button>
                )}
                {evs.length > 0 && (
                  <p className="mt-1 flex flex-wrap gap-1.5 t2-fs-meta text-[var(--t2-muted)]">
  ...
  return (
    <div>
      {controls}
      {eventForm}
      <div className="mt-4 flex overflow-hidden rounded-xl border border-[var(--t2-line)]">
          ...
          {LANES.map((lane, i) => {
            ...
            return (
              <div key={lane.key} className="flex flex-col justify-center border-b border-[var(--t2-line)] px-4" style={{ height: lane.h }}>
                <span className="t2-fs-body-sm font-bold text-[var(--t2-ink)]">{laneLabel(lane.key)}</span>
                {count > 0
                  ? <span className="t2-fs-meta text-[var(--t2-faint)]">{count}</span>
                  : lane.key !== "tournaments" && <span className="t2-fs-meta text-[var(--t2-faint)]">＋ {t("tour.tlAddLaneHint")}</span>}
              </div>
            );
          })}
        </div>
        ...
        <div ref={scrollRef} className="min-w-0 flex-1 overflow-x-auto">
          <div className="relative" style={{ width, height: INNER_H, backgroundImage: `repeating-linear-gradient(to right, rgba(0,0,0,0.05) 0, rgba(0,0,0,0.05) 1px, transparent 1px, transparent ${pxPerWeek}px)` }}>
            {/* Spur-Trennlinien */}
            {LANES.map((lane, i) => <div key={"sep" + lane.key} className="absolute left-0 right-0 border-b border-[var(--t2-text)]/[0.04]" style={{ top: laneTop(i) + lane.h }} />)}

            {/* Achse */}
            <div className="absolute inset-x-0 top-0 border-b border-[var(--t2-text)]/[0.06]" style={{ height: AXIS_H }} />
            {axisMode === "day" && days.map((ms) => {
              ...
              return (
                <div key={"ax" + ms} className="absolute top-2 flex flex-col items-center" style={{ left: xForMs(ms, bounds.startMs, pxPerDay), width: pxPerDay }}>
                  <span className="t2-fs-meta font-semibold uppercase text-[var(--t2-text-soft)]">{wdLetter(ms)}</span>
                  <span className={`mt-0.5 t2-fs-meta font-bold ${today ? "flex h-5 w-5 items-center justify-center rounded-full bg-[var(--t2-text)] text-[var(--t2-on-accent)]" : "text-[var(--t2-text-muted)]"}`}>{new Date(ms).getUTCDate()}</span>
                </div>
              );
            })}
            {axisMode !== "day" && (axisMode === "week" ? weeks : weeks.filter((_, i) => i % 4 === 0)).map((wk) => (
              <div key={"ax" + wk} className="absolute top-4 t2-fs-meta font-semibold text-[var(--t2-text-soft)]" style={{ left: xForMs(wk, bounds.startMs, pxPerDay) + 3 }}>{axisMode === "week" ? fmtShort(wk) : fmtMonth(wk)}</div>
            ))}

            {/* Heute-Linie (nur wenn im Bereich) */}
            {nowMs >= bounds.startMs && nowMs < bounds.endMs && (
              <div className="absolute z-10 w-px bg-[var(--t2-text)]/80" style={{ left: xForMs(nowMs, bounds.startMs, pxPerDay), top: AXIS_H, height: INNER_H - AXIS_H }} />
            )}

            {/* Fristen-Pills auf der Achse */}
            {deadlines.map((d) => {
              const x0 = xForMs(d.mondayMs, bounds.startMs, pxPerDay);
              if (d.kind === "unknown") return <div key={"dl" + d.id} className="absolute z-20 h-2.5 w-2.5 -translate-x-1/2 rounded-full border border-dashed border-[var(--t2-line-strong)]" style={{ left: x0 + pxPerDay * 3.5, top: AXIS_H - 14 }} title={t("tour.entryUnknownShort")} />;
              if (d.entryMs == null) return null;
              const x = xForMs(d.entryMs, bounds.startMs, pxPerDay);
              if (d.kind === "passed") return <div key={"dl" + d.id} className="absolute z-20 h-2 w-2 -translate-x-1/2 rounded-full bg-[var(--t2-surface-muted)] ring-1 ring-white" style={{ left: x, top: AXIS_H - 12 }} title={t("tour.entryExpired")} />;
              const daysLeft = Math.ceil((d.entryMs - nowMs) / DAY);
              const urgent = daysLeft <= 7;
              return (
                <button key={"dl" + d.id} type="button" onClick={() => setSelected(d.id)} className={`absolute z-20 -translate-x-1/2 rounded-full px-1.5 py-0.5 t2-fs-meta font-extrabold text-[var(--t2-on-accent)] shadow-sm ${urgent ? "bg-[var(--t2-warn)]" : "bg-[var(--t2-text)]"}`} style={{ left: x, top: AXIS_H - 18 }} title={t("tour.entryCountdown", { n: daysLeft })}>{t("tour.tlDeadlinePin", { n: daysLeft })}</button>
              );
            })}
            ...
            {LANES.map((lane, i) => lane.key === "tournaments" ? null : (
              ...
              >
                {(eventsByKind.get(lane.key) ?? []).map((ev) => {
                  ...
                  return (
                    <button key={ev.id} type="button" onClick={(e) => { e.stopPropagation(); setForm({ event: ev }); }} className="absolute top-1 flex max-w-[190px] items-center gap-1.5 rounded-full bg-[var(--t2-on-accent)] py-1 pl-1 pr-2 t2-fs-meta font-semibold text-[var(--t2-text-muted)] shadow-sm ring-1 ring-black/[0.08] hover:ring-matchup/40" style={{ left: x + 2 }} title={`${ev.title}${ev.event_time ? " · " + ev.event_time.slice(0, 5) : ""}`}>
                      <IconBadge k={ev.kind} tone={KIND_BADGE[ev.kind] ?? KIND_BADGE.other} />
                      {ev.event_time && <span className="shrink-0 text-[var(--t2-text-soft)]">{ev.event_time.slice(0, 5)}</span>}
                ...
                {(eventsByKind.get(lane.key)?.length ?? 0) === 0 && [7, 24].map((offDays) => {
                  ...
                  return (
                    <span key={"ghost" + lane.key + offDays} className="pointer-events-none absolute top-1 flex select-none items-center gap-1.5 rounded-full border border-dashed border-[var(--t2-line-strong)] bg-[var(--t2-on-accent)]/50 py-1 pl-1 pr-2.5 t2-fs-meta font-medium italic text-[var(--t2-text-soft)]" style={{ left: gx + 2 }}>
                      <span className="flex h-6 w-6 items-center justify-center rounded-full bg-[var(--t2-text)]/[0.03] text-[var(--t2-text-faint)]"><Glyph k={lane.key} /></span>
                      {laneLabel(lane.key)}
                    </span>
                  );
                })}
            ...
            {activeSeason.map((s) => {
              ...
              return (
                <button key={tt.id} type="button" onClick={() => setSelected(tt.id)}
                  className={`absolute flex items-center gap-1.5 overflow-hidden rounded-full bg-[var(--t2-on-accent)] py-1 pl-1 pr-2 text-left shadow-sm ring-1 ring-black/[0.08] transition hover:ring-matchup/50 ${selected === tt.id ? "ring-2 ring-matchup" : ""}`}
                  ...
                  {showFull ? (
                    <span className="min-w-0 flex-1"><span className="block truncate t2-fs-micro font-semibold leading-tight text-[var(--t2-text)]">{tt.city || tt.name || t("tour.fieldMissing")}</span><span className={`block truncate t2-fs-meta leading-tight ${st.text}`}>{tt.category || "—"}</span></span>
                  ) : showCity ? (
                    <span className="min-w-0 flex-1 truncate t2-fs-meta font-semibold text-[var(--t2-text-muted)]">{tt.city || shortCat(tt.category)}</span>
                  ) : null}
                  {showFull && <Face src={profile?.profileImage ?? null} name={profile?.firstName ?? null} />}
                  {tight.has(tt.id) && <span className="absolute right-1 top-0.5 t2-fs-meta text-[var(--t2-warn)]" title={t("tour.calTightArrival", { n: tight.get(tt.id)! })}>⚠</span>}
                </button>
              );
            })}
          </div>
      ...
      {/* Legende */}
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 t2-fs-meta text-[var(--t2-faint)]">
        ...
        <span className="flex items-center gap-1"><span className="rounded-full bg-[var(--t2-text)] px-1 t2-fs-meta font-bold text-[var(--t2-on-accent)]">N</span>{t("tour.tlLegendDeadline")}</span>
        <span className="flex items-center gap-1"><span className="text-[var(--t2-warn)]">⚠</span>{t("tour.tlLegendTight")}</span>
...
function DeadlineChip({ kind, entryMs, nowMs, t }: { kind: DeadlineKind; entryMs: number | null; nowMs: number; t: ReturnType<typeof useT> }) {
  if (kind === "unknown") return <span className="rounded-full border border-dashed border-[var(--t2-line-strong)] px-2 py-0.5 t2-fs-meta font-semibold text-[var(--t2-text-soft)]">{t("tour.entryUnknownShort")}</span>;
  if (kind === "passed" || entryMs == null) return <span className="t2-fs-meta font-semibold text-[var(--t2-text-soft)]">{t("tour.entryExpired")}</span>;
  const daysLeft = Math.ceil((entryMs - nowMs) / DAY);
  const urgent = daysLeft <= 7;
  return <span className={`rounded-full px-2 py-0.5 t2-fs-meta font-bold text-[var(--t2-on-accent)] ${urgent ? "bg-[var(--t2-warn)]" : "bg-[var(--t2-accent)]"}`}>{t("tour.entryCountdown", { n: daysLeft })}</span>;
}
    ...
    <div className="mt-4 t2-card">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="t2-fs-body font-bold text-[var(--t2-ink)]">{tt.city || tt.name || t("tour.fieldMissing")}{tt.country ? <span className="text-[var(--t2-faint)]">, {tt.country}</span> : null}</h3>
          <p className="mt-0.5 t2-fs-micro text-[var(--t2-muted)]">{fmtShort(wk)} – {fmtShort(wk + 6 * DAY)}{tt.category ? ` · ${tt.category}` : ""}</p>
        </div>
        <button type="button" onClick={onClose} className="shrink-0 t2-fs-micro font-semibold text-[var(--t2-faint)] hover:text-[var(--t2-ink)]">{t("common.close")}</button>
      </div>
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <DeadlineChip kind={dl?.kind ?? "unknown"} entryMs={dl?.entryMs ?? null} nowMs={nowMs} t={t} />
        {tightDays != null && <span className="t2-fs-meta font-semibold text-[var(--t2-warn)]">⚠ {t("tour.calTightArrival", { n: tightDays })}</span>}
      </div>
      <Link href="/tour2" className="mt-3 inline-flex t2-fs-micro font-bold text-[var(--t2-accent)] hover:underline">{t("tour.tlOpenInPlanner")} →</Link>
    </div>
  );
}
