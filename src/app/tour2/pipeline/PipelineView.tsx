...
export default function PipelineView() {
  ...
  const decisionSelect = (plan: TourSeasonPlanEntry) => (
    <select value={plan.decision} onChange={(e) => changeDecision(plan.id, e.target.value as TourDecision)} aria-label={t("tour.pipeColDecision")}
      className="rounded-full border border-[var(--t2-line-strong)] bg-[var(--t2-paper)] px-2.5 py-1 text-[12px] font-semibold text-[var(--t2-ink)] transition-colors hover:border-[var(--t2-ink)] focus:outline-none">
  ...
  if (weeks.length === 0) return <p className="mt-8 rounded-xl bg-[var(--t2-surface)] px-4 py-4 text-[14px] text-[var(--t2-muted)]">{t("tour.pipeEmpty")}</p>;

  return (
    <div className="mt-8">
      {/* Breit: echte Tabelle */}
      <div className="hidden overflow-hidden rounded-2xl border border-[var(--t2-line)] md:block">
        <table className="w-full border-collapse text-left text-[13px]">
          <thead>
            <tr className="bg-[var(--t2-surface)] text-[11px] font-bold uppercase tracking-[0.1em] text-[var(--t2-faint)]">
          ...
          <tbody>
            {weeks.map((w) => (w.isGap ? (
              <tr key={w.monday} className="border-t border-[var(--t2-line)]">
                <td className="whitespace-nowrap px-4 py-2.5"><span className="font-semibold text-[var(--t2-muted)]">{t("tour.pipeKw", { n: w.isoWeek })}</span><span className="ml-2 text-[11px] text-[var(--t2-faint)]">{fmtRange(w.monday)}</span></td>
                <td className="px-4 py-2.5 text-[12px] italic text-[var(--t2-faint)]" colSpan={5}>{t("tour.pipeGap")}</td>
              </tr>
            ) : w.items.map((it, idx) => (
              <tr key={it.plan.id} className="border-t border-[var(--t2-line)] align-top">
                <td className="whitespace-nowrap px-4 py-3">{idx === 0 && (<><span className="font-semibold text-[var(--t2-ink)]">{t("tour.pipeKw", { n: w.isoWeek })}</span><span className="ml-2 text-[11px] text-[var(--t2-faint)]">{fmtRange(w.monday)}</span></>)}</td>
                <td className="px-4 py-3"><span className="font-semibold text-[var(--t2-ink)]">{it.tour.city || t("tour.fieldMissing")}</span><span className="text-[var(--t2-faint)]">, {catName(it.tour.country)}</span><span className="ml-1.5 inline-block rounded-full bg-[var(--t2-surface)] px-2 py-0.5 text-[10.5px] font-semibold text-[var(--t2-muted)]">{it.tour.category || "—"}</span></td>
      ...
      <div className="space-y-2.5 md:hidden">
        {weeks.map((w) => (
          <div key={w.monday} className={`rounded-2xl p-3 ${w.isGap ? "border border-dashed border-[var(--t2-line-strong)]" : "border border-[var(--t2-line)]"}`}>
            <div className="flex items-baseline justify-between">
              <span className={`text-[12px] font-bold ${w.isGap ? "text-[var(--t2-faint)]" : "text-[var(--t2-ink)]"}`}>{t("tour.pipeKw", { n: w.isoWeek })}</span>
              <span className="text-[11px] text-[var(--t2-faint)]">{fmtRange(w.monday)}</span>
            </div>
            {w.isGap ? (
              <p className="mt-1 text-[12px] italic text-[var(--t2-faint)]">{t("tour.pipeGap")}</p>
            ) : w.items.map((it) => (
              <div key={it.plan.id} className="mt-2 border-t border-[var(--t2-line)] pt-2 [&:nth-child(2)]:mt-1.5 [&:nth-child(2)]:border-0 [&:nth-child(2)]:pt-0">
                <p className="text-[14px] font-semibold text-[var(--t2-ink)]">{it.tour.city || t("tour.fieldMissing")}<span className="text-[var(--t2-faint)]">, {catName(it.tour.country)}</span></p>
                <p className="text-[11px] text-[var(--t2-muted)]">{it.tour.category || "—"}</p>
                <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1.5">
                  {pill(it.plan)}
                  <DeadlineCountdown tournament={it.tour} now={nowMs} />
                  {weekCostMinor != null && <span className="text-[12px] text-[var(--t2-muted)]">{money(weekCostMinor)}</span>}
                </div>
