...
/**
 * Anmeldemaske für /tour2. Schwarz/Weiß + Matchup-CTA. Dieselbe useAuth()-Session.
 */
export default function TourLoginCard() {
  ...
  const inp = "w-full border border-white/20 bg-black px-3.5 py-2.5 text-[14px] text-white placeholder:text-neutral-500 focus:border-white focus:outline-none";
  ...
  return (
    <div className="flex h-[100dvh] flex-col items-center justify-center bg-black px-6 text-white">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-2 text-sm text-neutral-400">{t("tour.loginFormLead")}</p>
        <form onSubmit={submit} className="mt-6 space-y-2.5">
          ...
          {error && <p className="text-[12px] font-semibold text-white">{error}</p>}
        ...
        <div className="mt-4 flex items-center justify-between text-[12px]">
          <Link href="/reset-password" className="font-semibold text-neutral-500 hover:text-white">{t("tour.loginForgot")}</Link>
