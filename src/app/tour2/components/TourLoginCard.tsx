  ...
  return (
    <div className="flex h-[100dvh] flex-col items-center justify-center bg-[var(--t2-paper)] px-6 text-[var(--t2-ink)]">
      <div className="w-full max-w-sm">
        ...
        <h2 className="t2-display mt-3 t2-fs-display">{t("tour.loginRequiredTitle")}</h2>
        <p className="mt-2 t2-fs-body text-[var(--t2-muted)]">{t("tour.loginFormLead")}</p>
        <form onSubmit={submit} className="mt-6 space-y-2.5">
          ...
          {error && <p className="t2-fs-micro font-semibold">{error}</p>}
        ...
        <div className="mt-4 flex items-center justify-between t2-fs-micro">
          <Link href="/reset-password" className="font-semibold text-[var(--t2-text-soft)] hover:text-[var(--t2-text)]">{t("tour.loginForgot")}</Link>
          <Link href="/app" className="font-semibold text-[var(--t2-accent)] hover:underline">{t("tour.loginToApp")}</Link>
        </div>
      </div>
    </div>
  );
