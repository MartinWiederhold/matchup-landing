...
export default function TournamentDetail({
  ...
  const noteCls = "font-semibold text-[var(--t2-muted)] underline";
  const entryHint = (
    <>
      ...
      {tt.website && (
        <p>
          ...
          <span className="mt-0.5 block font-normal text-[var(--t2-faint)]">{t("tour.entryWebsiteNote")}</span>
        </p>
      )}
      {isWta ? (
  ...
  const visaProvInner = visaInfo && (
    <>
      ...
      <p className="mt-1 font-semibold text-[var(--t2-muted)]">{t("mode.visaNatConsulate")}</p>
    </>
  );
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--t2-paper)]">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <div className="divide-y divide-[var(--t2-line)]">
          ...
          {planId && (
            <div className="py-2.5">
              ...
              {entryOpen && (
                <div className="mt-2 space-y-2.5 rounded-xl border border-[var(--t2-line)] bg-[var(--t2-surface)] p-3">
                  {/* Status-Auswahl */}
          ...
          <div className={`flex items-center justify-between gap-3 ${hasCountdown ? "py-3" : "py-2.5"}`}>
            <span className={hasCountdown ? "text-[12px] font-bold uppercase tracking-[0.1em] text-[var(--t2-muted)]" : "text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--t2-faint)]"}>
              ...
              {tt.series === "wta" && <span className="ml-1.5 normal-case tracking-normal font-medium text-[var(--t2-faint)]">· {t("tour.entryMayVary")}</span>}
            </span>
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 space-y-1.5">
                {demoShown.map((d) => (
                  ...
                  <button key={d.id} type="button" onClick={() => setDemoSelected(d)} className="flex w-full items-center gap-2.5 rounded-xl border border-[var(--t2-line)] px-2.5 py-2 text-left transition-colors hover:bg-[var(--t2-surface)]">
            ...
            <>
              ...
              <div className="mt-2 space-y-1.5">
                {provCapped.map((p) => {
                  ...
                  return (
                    <div key={p.id} className="overflow-hidden rounded-xl border border-[var(--t2-line)]">
                      <button type="button" onClick={() => toggleProv(p.id)} aria-expanded={open} className="flex w-full items-center gap-2.5 px-2.5 py-2 text-left">
                        ...
                        <svg viewBox="0 0 24 24" aria-hidden className={`h-4 w-4 shrink-0 text-[var(--t2-faint)] transition-transform ${open ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6" /></svg>
                      </button>
                      {open && (
