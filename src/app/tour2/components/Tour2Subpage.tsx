...
export default function Tour2Subpage({
  ...
  return (
    <div className={`mx-auto px-4 py-6 pb-28 sm:px-6 ${wide ? "max-w-[1800px]" : "max-w-[1280px]"}`}>
      ...
      <h1 className="mt-1 text-[28px] font-extrabold tracking-tight text-white sm:text-[34px]">{title}</h1>
      <p className="mt-2 max-w-2xl text-[14px] leading-relaxed text-neutral-400">{subtitle}</p>
      <BackToWorkspace />
      <div className="mt-6 overflow-hidden rounded-3xl bg-[#f4f4f5] p-4 text-neutral-900 ring-1 ring-white/10 sm:p-6">
        {children}
      </div>
    </div>
  );
