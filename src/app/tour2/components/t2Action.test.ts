...
describe("tour2ActionHref", () => {
  it("mappt Domain-Routen /tour/* auf kanonische /tour2-Flächen", () => {
    ...
    expect(tour2ActionHref("/tour/browse")).toBe("/tour2/finder");
    expect(tour2ActionHref("/tour/map")).toBe("/tour2/finder");
...
describe("tour2PlannerTournamentHref", () => {
  it("öffnet das Turnier im Planer", () => {
    expect(tour2PlannerTournamentHref("abc")).toBe("/tour2/season?id=abc");
    expect(tour2PlannerTournamentHref("a b")).toBe("/tour2/season?id=a%20b");
  });
});
