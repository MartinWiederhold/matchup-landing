...
/**
 * Untere Tableiste für /tour2 unter 768px — vier Slots: Heute, Suchen, Saison, Mehr.
 */
...
export type T2Tab = {
  ...
  label: string;
  href?: string;
  icon: "overview" | "finder" | "season" | "more";
  badge?: number | null;
};

function Glyph({ name, size }: { name: T2Tab["icon"]; size: number }) {
  const p = { width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round" as const, strokeLinejoin: "round" as const, "aria-hidden": true as const };
...
export default function Tour2TabBar({
  ...
  const icon = dense ? 16 : 23;
  return (
    <div
      className={`t2-tabs pointer-events-none fixed inset-x-0 bottom-0 z-30 flex justify-center px-4 pb-[max(10px,env(safe-area-inset-bottom))] md:hidden ${
        compact ? "t2-tabs-compact" : ""
      } ${dense ? "is-dense" : ""}`}
    >
      <nav
        className={`pointer-events-auto flex w-full items-center justify-between rounded-full bg-[var(--t2-on-accent)] shadow-[0_16px_50px_-12px_rgba(0,0,0,0.25)] ring-1 ring-black/10 transition-[max-width] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          dense ? "px-1.5 py-1" : "px-3 py-2.5"
        } ${compact ? (dense ? "max-w-[200px]" : "max-w-[280px]") : dense ? "max-w-[236px]" : "max-w-[340px]"}`}
        aria-label="Tour"
      >
        {tabs.map((tab) => {
