...
- **Datenimport = Code ZUERST deployen, Daten DANACH schreiben.** Die Seite ist öffentlich (Gate am 09.08. entfernt, Commit `6050fbb`); `/tour` & `/tour/browse` sind für jeden eingeloggten Nutzer erreichbar (`COMPETE_EARLY_ACCESS_OPEN` gatet nur den Umschalter in `/app`, nicht die `/tour/*`-Routen). Die Prod-DB ist geteilt — **neue Zeilen gehen SOFORT live, ohne Deploy, ohne Prüfung.** Führt ein Import neue Serien/Kategorien/Felder ein, die die UI anders darstellt (Beispiel MU-043: 392 Junioren gingen mit falschen Fristen Do −18 statt Di −20 und Label „ATP Challenger" live, weil der Code erst später deployt war): (1) UI/Domain-Code deployen, der die neuen Daten korrekt rendert, (2) ERST DANN `--write` in die Prod-DB, (3) Prod prüfen. Nie Daten vor dem Code. Siehe MU-044.
...
- **Regelwerk-Zitate gehören VOLLSTÄNDIG in den Kommentar — nicht gekürzt, keine Auslassungspunkte in der Mitte.** Wenn eine Regel aus einem Regelwerk (ATP/WTA/ITF) in den Code kommt, wird der maßgebliche Satz wörtlich und ganz zitiert. Grund: Bei der Punkte-Zählgrenze war das Zitat in `punkte-tabellen-report.md` vor der entscheidenden Umwandlungsklausel abgeschnitten (`… and …`). Daraus entstand eine falsche Regel (best 6/7 statt 18/19), die monatelang für Herren UND Damen zu niedrig rechnete — ohne sichtbares Symptom, weil die Zahl plausibel aussah (MU-047). Ein `…` mitten in einer Regel ist ein **Warnsignal**: dort steht womöglich die Bedingung, auf die es ankommt — im Zweifel die Primärquelle vollständig nachlesen, bevor du daraus Logik ableitest.
