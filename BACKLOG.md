...
### MU-054 · Meldeweg — Direktlink zur Meldung geprüft, beste erreichbare Lösung gebaut · S · **erledigt**
**Befund (Adressstruktur geprüft, ohne Anmeldung):** Es gibt bei **keiner Tour** einen Direktlink zur Meldung. Der `tournamentLink` der ITF führt auf die **öffentliche Turnierseite** (itftennis.com/en/tournament/…), nicht zur Meldung. Die **Tour Zone** trägt **keine Turnier-Kennung** in der Adresse und leitet nach dem Login **nicht zurück** (kein returnUrl). **WTA PlayerZone** ebenso — die OAuth-Rückadresse (`redirect_uri`) zeigt auf den **blanken App-Root**, nicht auf das Turnier. **ATP** nicht prüfbar wegen **KI-Sperre** (`atptour.com` robots).
**Erledigt:** die beste erreichbare Lösung — für ITF-Turniere verweist der Meldeweg **zuerst auf die öffentliche Turnierseite** (richtiges Turnier + Fact Sheet + „Login to Tour Zone"-Knopf), **darunter der Portal-Link**. Beschriftung ehrlich („Turnierseite mit Meldeinfo", kein „Jetzt anmelden") + Hinweis, dass die Seite nicht selbst anmeldet. Die URL steckt als `source_url` in den ITF-Endpunkt-Claims; `resolve-tournaments.mjs` hebt sie für `itf_wtt`/`itf_juniors` in die Spalte `website` (nur exakt `itftennis.com/en/tournament/…`, nie Wikipedia/OSM/WTA-API) — selbstheilend, sobald der Endpunkt-Import auf `main` läuft. Backfill: 392/392 aktive Junioren + 409 itf_wtt; WTA/Challenger bleiben NULL. Im Code (`EntryDeadline.tsx`) festgehalten, dass es keinen Deep-Link/returnUrl gibt — damit später niemand danach sucht.
...
**Wirkung:** Die Rangprognose für Herren war **systematisch zu niedrig**. Bei zehn gleichmäßigen Ergebnissen fehlten 40 %; bei 25 Turnieren im Jahr zählte weniger als ein Drittel. Dieselbe Fehlerklasse wie bei den Damen (MU-045), nur seit Anfang unbemerkt und für die andere Nutzerhälfte.
**Erledigt:** `scorePoints` (v4) setzt die Herren-Zählgrenze auf **18/19** (best 6/7 + 12 nicht gespielte Pflichtturniere; die App erfasst nur ITF/Challenger, nie ein GS/Masters-Hauptfeld → immer +12). Die **vollständige** 9.03-B-Klausel steht jetzt als Kommentar in `points.ts` (nicht gekürzt). `punkte-tabellen-report.md` korrigiert, damit das abgeschnittene Zitat den Fehler nicht erneut auslöst. Test aus dem Bericht: zehn Ergebnisse → **Summe 107 statt 92**; Cap-Test (20 → best 18). 260 Tests grün.
**🚫 Sperre (aufgehoben):** ~~Nicht mit echten Nutzern starten, solange die Herren-Prognose falsch rechnet.~~ (Fix gebaut; nach Deploy live.)
...
**Zielgruppe trennbar (ja):** Spalte **„Offen für"** (Sample: 30× „Deutschland" offen vs. 6× „eigener Verein" = Clubmeisterschaft, ausschließen) + **Konkurrenz** (22× Herren Einzel vs. Junioren U12/U14, Herren 30/50) + **LK**-Bereich (z. B. „18,0–25,0").
**Meldefrist — die Kernfrage: JA, aber eine Ebene tiefer.** NICHT in der Liste; auf der Detailseite (`tennis.de/…/turniersuche.html#detail/<id>`) im DOM bestätigt: **Meldeschluss, Nenngeld (Meldegebühr), Belag (Sand/Halle), Auslosung, Ausschreibung**. Damit **erster Kanal mit gelesener statt gerechneter Frist**.

**Nationale API? NEIN (geprüft).** Die tennis.de-Turniersuche ist ein **ZK-Server-Widget** (`widgets.tennis.de/widgets/layout/tournament/trnsearchrs.zul`), das über das zustandsbehaftete **`zkau`-Protokoll** kommuniziert — keine saubere REST/JSON-Schnittstelle. Und der Host **`widgets.tennis.de` ist robots `Disallow: /`** (komplett gesperrt). Die Adobe-I/O-API `928874-dtbsearch.adobeioruntime.net/api/v1/web/tennis/search` ist nur die **Volltext-Seitensuche** (Felder `body_content, title, url, meta_*`), keine Turnierdaten. → **Kein erlaubter nationaler Zugang; es bleiben die ~18 Regional-`liga.nu`-SSR-Seiten.**

**Was ein Parser leisten muss:** (1) je Region `tournamentCalendar` laden, wochenweise über `&date` blättern, 5-Spalten-Tabelle parsen (deutsche Formate „18,0", Datumsbereiche „13.04. bis 27.09.", `#detail/<id>`); filtern „Offen für" ≠ eigener Verein, Konkurrenz ∈ Herren/Damen-Einzel. (2) je `<id>` Detailseite lesen → Meldeschluss/Nenngeld/Belag. (3) **503-Backoff** (liga.nu-Backend war während der Prüfung zeitweise `503`, nginx/Varnish, keine Bot-Wall). (4) Region-Dedup, LK-/Konkurrenz-Mapping auf das eigene Serien-/Kategoriemodell.
**Fazit:** Brauchbar und mit echter Frist — aber deutlich mehr Aufwand als ein JSON-Endpunkt (SSR, zwei Stufen Liste→Detail, ~18 Regionen, flakiger Backend). Kein Shortcut über eine nationale API.
